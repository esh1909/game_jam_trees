...
var max_water: float = 0.0: set = _set_max_water, get = _get_max_water
...
func _set_water(value:float) -> void:
		water = lamp(value, 0, max_water)
		if water_ui_full != null:
			water_ui_full.set_size(Vector2(value * 32, 32))
...
func on_event_health_changed(node: Node, new_health: float):
