class_name WaterController extends Node
...
func _ready() -> void:
	EventController.connect("on_water_ui_ready", on_event_health_ui_ready)
...
func on_event_health_ui_ready(node: Node) -> void:
	EventController.emit_signal("on_water_changed", node, health)
	_start_drop = true
...
func _set_health(value: float) -> void:
	...
	if value <= 0 and parent and not parent.dead:
		...
		# print('health: ', value)
		if parent:
			EventController.emit_signal("on_water_changed", parent, value)
		health = value
